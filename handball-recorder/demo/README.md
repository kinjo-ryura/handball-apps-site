...
- **クエリパラメータを分けた**。試合とハイライトは配信上も別コレクションで slug の名前空間が
  ...
  扱っている（判断の経緯は親リポの学習サイクル文書にある）。#231 でパス方式へ移す際も
  この 2 系統をそのまま別パスに写せる。
...
- ⚠ **ただし届くのは LINE だけ。X の WebView はカスタムスキームも渡さない**
  （2026-08-27 実測・#249。端末別の実測記録は親リポの学習サイクル文書にある）。
  X のタイムライン投稿は URL が `t.co` に置き換わるため UL も効かず、**X 経路では
  アプリへ渡す手段が無い**。ボタンは表示されるが押しても無反応で、**導入済みか未導入かも
  問わない**（E-1 の Safari と違いエラーも出ない）。X の WebView は UA に `Twitter` を
  含む、と当初書いたが**これは誤り**（iOS の X アプリ内ブラウザは UA に識別子を持たず、素の
  Safari と区別できない）。**UA による出し分けはできない**。この節の「なぜ要るか」は
  **LINE についてのみ成り立つ**
- **App Card（`twitter:app:*`）も効かなかった**（2026-08-28 実測・#257。同 F / G 節）。
  X 自身がカードからアプリを開く経路は生きており（YouTube は `twitter:app:url:iphone` の
  `vnd.youtube://` で実際にアプリが開く）、同じ meta を試合 / ハイライトページに入れたが、
  **`summary_large_image` との併記では読まれない**。**ただしカードは化けず OG 画像は保たれた**ので、
  meta は残してある（`tools/generate-match-pages`）。X の挙動が変わったときに効く可能性がある
...
- **主従は塗り分けで示す。** App Store が主（塗り）、アプリで開くが従（`secondary` の
  ...
  出し分けではなく併置で扱っている
- **押しても開かなかったときだけ注記を出す**（`.demo-cta-note` / `data-demo-app-open-note`・#257）。
  押下から **1.2 秒**後に画面が離れていなければ「アプリが開きませんでした」を出す。
  **経路ではなく結果で判定している** — X のアプリ内ブラウザだけを狙って隠したいが、
  **iOS の X は UA に識別子を持たず素の Safari と区別できない**ので、UA で隠すと
  **ボタンが正しく動く Safari の読者まで道連れ**になる。押した後に画面が離れたかどうかなら、
  経路を知らなくても判定できる。LINE と素の Safari ではアプリが開いて背面に回るので出ない。
  未導入の Safari では E-1 のエラーの後に出るので、**宛先の明示も兼ねる**
- ⚠ **自動で App Store へ飛ばさないこと。** タイマー式フォールバック（押して N 秒後に
  App Store へ遷移）は **アプリが開いた場合にも発火して勝手に飛ぶ**既知の壊れ方があり、
  #230 の実装時に意図的に避けた。注記を出すだけなら、誤検出しても害は注記が 1 つ出るだけ
- **`leftForApp` を消さないこと。** タイマー発火時の `visibilityState` だけで判定すると、
  **アプリを開いてすぐ戻ってきた読者に「開きませんでした」と出る**（visible に復帰しているため）。
  一度でも離れたかを `visibilitychange` / `pagehide` で覚えておく
...
**この遅延ロードはブラウザ自動化（claude-in-chrome）では検証できない。** Chrome は
**バックグラウンドタブに IntersectionObserver のコールバックを配送しない**ので、
LP の埋め込みは「読み込み中…」のまま止まって見える。**壊れているように見えるが正常**。
2026-08-27 に本番・ローカルの両方で踏み、バグとして起票しかけた。

- 見分け方は `document.visibilityState` を見る。`"hidden"` なら環境側の制約
- 決め手は「**その場で新しく登録した IO も発火しない**」こと。表示中・ビューポート内の
  要素に対して発火しなければ、原因はページのコードではない
- `mount()` 自体は健全かを切り分けるなら、コンソールから
  `(await import('./demo/demo.js?probe=1')).mount(document.querySelector('[data-demo]'))`
  を直接呼ぶ。これで描画されればローダの問題（＝この制約）に絞れる
- **LP を実際に検証するなら人が前面のタブで開いて層を展開する**。自動化で見られるのは
  遅延ロードを使わないページ（デモページ・`demo/list/`・個別ページ）だけ
...
- この site は GitHub Actions を持たない素の静的 Pages。別リポの `handball-toolkit` をクロスリポ CI でビルドするのは過剰。
...
現行の `wasm/` は handball-toolkit `72c1024` からビルド（2026-09-04 に同じコミットから再ビルドして差し替え。下記）。コア更新のたびにこの provenance を更新すること（iOS アプリと同じコアで動かすため、古いビルドを混ぜない）。

### ビルド機の絶対パスを埋めない（handball-project#285）

panic の位置情報として依存クレートのソースパスが `.wasm` に残るため、素直にビルドすると
**公開バイナリから開発機の macOS ユーザー名が読める**。2026-09-02 のセキュリティ監査で
`/Users/<ユーザー名>/.cargo/registry/...` が 15 本埋まっていることが分かり、
`build_wasm.sh` が `--remap-path-prefix` で畳むようにした（同スクリプトの最終ステップが
畳み込みの有無を検査し、残っていればビルドを失敗させる）。**この検査を外さないこと。**

2026-09-04 の差し替えは**コアの更新ではない**。provenance の `72c1024` から畳み込みだけを
足して再ビルドしたもので、JS グルーと `.d.ts` は 3 本ともバイト単位で一致し、`.wasm` は
埋め込み文字列が短くなったぶん 192 バイト小さいだけ。配信中の試合 46 件・ハイライト 6 件の
